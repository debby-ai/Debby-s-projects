		...
		if(empty($_POST['deadline'])) {
			throw new Exception('Deadline can not be empty');
				...
				if(($user != "") && ($utype_db!="teacher"))
				{
					...
					$result = mysqli_query($con, "INSERT INTO post (postby_id,subject,class,medium,salary,location,deadline) VALUES ('$user','$sublist','$classlist','$mediumlist','$_POST[sal_range]','$_POST[location]','$_POST[deadline]')");
...
<center><div class="post_form">
    ...
    <div class="location">
        <h3>Location</h3>
        <div class="lc">
            <?php echo '<input type="text" id="location" name="location" value="'.$f_loca.'" placeholder="e.g: Kenya,Nakuru">'; ?>
        </div>
    </div>
